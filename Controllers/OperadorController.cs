...
namespace TransportationCore.Controllers
{
    ...
    public class OperadorController : ControllerBase
    {
        ...
        public OperadorController(ApplicationDbContext context, IMapper mapper, ISoftDeleteService softDeleteService)
        {
            _context = context;
            this.mapper = mapper;
            this.softDeleteService = softDeleteService;
            this.tipoEmpleado = (int)EnumTipoEmpleado.Operador;
        }


        [HttpGet]
        public async Task<ActionResult<IEnumerable<OperadorConsultarDto>>> GetEmpleados()
        {
            ...
            var operador = await _context.Empleados.Include(x => x.Municipio).Include(x => x.Municipio.Estado).ToListAsync();
        ...
        [HttpGet("{id}")]
        public async Task<ActionResult<OperadorConsultarDto>> GetOperador(long id)
        {
            if (_context.Empleados == null)
            {
                return NotFound();
            }
            var operador = await _context.Empleados.Include(x => x.Municipio).Include(x => x.Municipio.Estado).FirstAsync(b => b.IdEmpleado == id);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> PutOperador(long id, OperadorActualizarDto operadorDto)
        {
            ...
            if (ValidarNumeroContrato(operadorDto.IdEmpleado, operadorDto.NumeroContrato))
        ...
        [HttpPost]
        public async Task<ActionResult> PostOperador(OperadorCrearDto operadorDto)
        {
